    ...
    for(int i = 0; i < T; i ++){
        ...
        // for(int j = 0; j <= i; j ++){
            for(int si: insertables[i]){// - j]){
                ...
                int newVal = howWeGetHere[i] + 1; // build off previous + 1
                for(int k = 1; k <= len; k ++){
                    int writeTo = i + k;
                    if(writeTo > T) break; // out of bounds write
                    if(howWeGetHere[writeTo] > newVal){
                        // cout << "DP " << i << " -> " << writeTo << " writing " << newVal << " used " << substrs[si] << endl;
